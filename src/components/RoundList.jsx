...
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import { useNavigate } from 'react-router-dom';
import { fetchCurrentRound, fetchRoundResult, setManualRoundOutcome } from '../services/api';
import ResultForm from './ResultForm';
...
function RoundList() {
  ...
  const { data: recentRounds, isLoading: isRecentLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      console.log('Fetching periods:', periods.map(p => ({ period: p, time: new Date(parseInt(p.split('-')[1])).toISOString() })));
      const results = await Promise.all(
        periods.map(async (period) => {
          try {
            const result = await fetchRoundResult(period);
            if (!result) return null;
            return {
              period: result.period,
              expiresAt: result.expiresAt,
              resultNumber: result.result.number,
              resultColor: result.result.color,
            };
          } catch (err) {
            console.warn('fetchRoundResult failed for period:', period, err);
            return null;
          }
        })
      );
  ...
  const setManualOutcomeMutation = useMutation({
    ...
    onSuccess: (result, { period }) => {
      ...
      queryClient.setQueryData(['currentRound'], (old) =>
        old && old.period === period ? { ...old, result: { number: result.resultNumber, color: result.resultColor } } : old
      );
      setError('');
      closeManualSetModal();
    },
    ...
    ? [
        {
          period: currentRound.period,
          expiresAt: currentRound.expiresAt,
          resultNumber: currentRound.result?.number,
          resultColor: currentRound.result?.color,
        },
        ...(recentRounds || []),
      ]
    : recentRounds || [];
